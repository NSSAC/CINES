...
export const ModifyPermissions = ({ showModal, handleClose, props }) => {
  ...
  function addUser(event, value) {
    ...
    values.push({
      ...
      computeACL:
        selectedItem.computeACL.indexOf(value.id) !== -1 ? true : false,
  ...
  function onSubmit() {
    ...
    var allNewUsers = users.map((user) => {
      var perm = [];
      if (
        selectedItem.readACL.indexOf(user.id) !== -1 ||
        selectedItem.writeACL.indexOf(user.id) !== -1 ||
        selectedItem.computeACL.indexOf(user.id) !== -1
      ) {
        perm = ["read", "compute", "write"];
        fileServiceInstance
          .revoke(path + selectedItem.name, perm, user.id, false)
          .then(() => {
            perm = [];
            ...
            fileServiceInstance
              .grant(path + selectedItem.name, perm, user.id, false)
              .then((response) => {
  ...
  useEffect(() => {
    ...
    userServiceInstance
      .queryUsers(
        `or(eq(id,re:${searchValue}),eq(first_name,${searchValue}),eq(name,${searchValue}))&limit(5)`
      )
      .then((response) => {
