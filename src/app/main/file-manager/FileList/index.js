...
function FileList(props) {
    ...
    React.useEffect(()=>{
        if (file_removal && file_removal.removal_completed && !showNotification){
            setShowNotification(`File removal completed.  ${file_removal.files_removed} files deleted.`)
            setSelected({})
    ...
    const table_columns = [
        {
            ...
            "cellClass": "",
            "formatter": (val,obj)=>{
                return <Link style={{ color: '#1565C0' }} title={val} to={val}>{val}</Link>
    ...
    React.useEffect(()=>{
        ...
        setContainerActions(
            <React.Fragment>
                ...
                {canWrite && (<Tooltip title="Create a new folder" aria-label="add">
                ...
                {canWrite && (<Tooltip title="Upload files" aria-label="add">
    ...
    React.useEffect(()=>{
        if (files){
            setTotalFileCount(filtered_files?filtered_files.filtered.length:files.length)
        }
        // setSelected({})
    ...
    React.useEffect(()=>{
        ...
        function confirmAndDelete(selectedIds, files){
            const selFiles = selectedIds.map((id)=>{
                var found;
                if (files.some((f)=>{
                    if (f.id===id){
                        found=f;
            ...
            if (selFiles.length !== selectedIds.length){
                console.warn("Unexpected SelectedIDs !== selFiles.length", selectedIds, selFiles)
            }

            confirmAlert({
                title: 'Confirm',
                files: selFiles,
                onConfirm: (ids) => { dispatch(Actions.deleteFiles(ids)) },
                closeOnClickOutside: false
            })
        }
        ...
        if (_files && _files.length>0 && selectedIds.length===1 ){
            ...
            setFileActions(        
                <React.Fragment>
                    {canDownload && (<Tooltip title="Download selected file(s)" aria-label="add">
                    ...
                    {canWrite && (<Tooltip title="Rename file or folder" aria-label="add">
                        <IconButton className="w-64 h-64" onClick={()=>setShowRenameDialog(true)}>
                            <Icon className="text-white text-4xl">
                                edit
                            </Icon>
                        </IconButton>
                    </Tooltip>)}
                    {canWrite && (<Tooltip title="Move file or folder" aria-label="add">
                    ...
                    {canWrite && (<Tooltip title="Delete selected file or folder" aria-label="add">
            ...
            setFileActions(        
                <React.Fragment>
                    ...
                    {canWrite && (<Tooltip title="Delete selected files or folders" aria-label="add">
                        <IconButton className="w-64 h-64"   onClick={()=>{confirmAndDelete(selectedIds,_files)}}>
                            <Icon  className="text-white text-4xl">
                                delete
                            </Icon>
    ...
    if (_files && _files.length>=0){
        ...
        return (
            <div className="overflow-auto w-full h-full">
                <div className="flex w-full h-full">
                    <div className="flex-row flex-grow h-full w-full overflow-auto" onDragEnter={e=>onDragEnter(e)} onDragOver={e=>onDragOver(e)} onDragLeave={e=>onDragLeave(e)} onDrop={e=>handleDrop(e)}>
                        <Table stickyHeader className='fileTableStyle  max-h-full'>
                            <FileListHeader 
                                ...
                                onSort={(sort)=> { 
                                    setSort(sort)
                                    localStorage.setItem("file_manager_sort",JSON.stringify(sort))
                                }}
                                toggleAll={toggleAll}
                            ...
                            {(_files.length>0)&&(
                                <TableBody onClick={onClickRow} >
                                    {_files && _files.map((f)=><FileRow enableCheckBoxes={props.enableCheckBoxes||false} key={f.id} table_columns={table_columns} selected={selected[f.id]||false} meta={f} />)}
                                </TableBody>
                            )}
                            {(_files.length===0)&&(
                                <TableBody onClick={onClickRow} >
                                    <TableRow><TableCell colSpan={20} className="text-center text-lg font-semibold">
                                        {filtered_files?(<span>No matching files</span>):(<span>Empty Folder</span>)}
                                    </TableCell></TableRow>
                                </TableBody>
                            )}
                    ...
                    {showDetailPanel && (
                        ...
                        <div className="flex flex-row flex-initial min-w-min md:w-1/3 h-full">
                            <div className="flex-row w-1 flex-initial" style={{background: "#999"}}></div>
                ...
                />)}
                {showMoveFiles && (<MoveMultiple
                    showModal={showMoveFiles}
                    sourceIDs={selectedIds}
                    props={props}
                    path={props.path}
                    handleClose={()=>{setShowMoveFiles(false)}}
       ...
       return ( <div className="flex flex-1 flex-col items-center justify-center mt-40"  onDragEnter={e=>onDragEnter(e)} onDragOver={e=>onDragOver(e)} onDragLeave={e=>onDragLeave(e)} onDrop={e=>handleDrop(e)}>
