...
function CSVViewer(props) {
    ...
    return (
        <div className="flex flex-col overflow-hidden h-full ">
            ...
            {selectedTab === 1 && data &&  <div className={`${enableFileView?'':'hidden'} p-4 overflow-auto flex-grow`}><pre>{data.raw}</pre></div>}
            ...
            {selectedTab === 4 && data && <div className={`${enableFileView?'':'hidden'} p-4 overflow-auto flex-grow`}>
                ...
                <Table stickyHeader>
                    {hasHeader && (
                        <TableHead >
                            <TableRow>
                                {Object.keys(data.parsed.data[0]).map((key,idx)=>{
                                    const val = data.parsed.data[0][key]
                                    return (
                                        <TableCell key={idx}>
                                            {val}
                                        </TableCell>
                    ...
                    <TableBody>
                    {(hasHeader?data.parsed.data.slice(1):data.parsed.data).map((row,ridx)=>{
                        return (
                            <TableRow key={ridx}>
